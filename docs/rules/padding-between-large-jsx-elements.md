...
The rule accepts an object with a `minLines` property:
...
`minLines` must be an integer from `0` through `999`. It defaults to `8` when the option is omitted. Both adjacent elements must meet the threshold. The opening and closing lines are included in an element's line count.
